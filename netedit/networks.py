...
"""
        ...
        f.write(
            f"""\t<edge id="0r" from="beg" to="0" numLanes="1" priority="78" speed="20.11" />\n"""
        )
        edges.append("0r")
        for i in range(1, num_intersections):
            f.write(
                f"""\t<edge id="{i}r" from="{i-1}" to="{i}" numLanes="1" priority="78" speed="20.11" />\n"""
            )
            edges.append(f"{i}r")

        f.write(
            f"""\t<edge id="{num_intersections}r" from="{num_intersections - 1}" to="end" numLanes="1" priority="78" speed="20.11" />\n"""
        )

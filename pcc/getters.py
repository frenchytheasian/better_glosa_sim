...
def get_state() -> Dict:
    ...
    vehicles = [vehicle for vehicle in vehicles if vehicle.startswith("pcc_")]
...
def _get_single_schedule(tlLogic: ET.Element) -> List[int]:
    ...
    schedule = []
    r = (
        int(phases[0].attrib["duration"])
        if phases[0].attrib["state"] == "r"
        else int(phases[1].attrib["duration"])
    )
    g = (
        int(phases[0].attrib["duration"])
        if phases[0].attrib["state"] == "G"
        else int(phases[1].attrib["duration"])
    )

    current = (
        0 if phases[0].attrib["state"] == "G" else int(phases[0].attrib["duration"])
    )
    while current < N:
